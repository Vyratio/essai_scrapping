...
class MonCrawlSpiderSerieSpider(scrapy.Spider):
    name = 'mon_crawl_spider_serie'
    ...
    rules_serie=(
        Rule(LinkExtractor(restrict_xpaths='//td[@class="titleColumn"]/a'),callback='parse',follow =True),
    )
    ...
    def parse(self,response):
        ...
        yield{
            ...
            'Nom du film' : response.xpath('//td[@class="titleColumn"]/a/text()').get(),
            'Annee' : response.xpath('//span[@class="secondaryInfo"]/text()').get(),#marche dans le shell voir video Scraping the top 250 movie ratings in IMDB (part 1) by Python
            'Score' : response.xpath('//td[@class="ratingColumn imdbRating"]/strong/text()').get(),
